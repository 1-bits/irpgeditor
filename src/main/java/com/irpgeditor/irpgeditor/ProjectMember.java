...
import com.irpgeditor.irpgeditor.tree.NodeAbstract;
...
/**
 * Represents a Source member in a project and the members settings. For
 * example, how to compile it, and its compile options.
 * 
...
public class ProjectMember extends NodeAbstract implements ListenerMember {
	...
	Icon icon;
	Logger logger = LoggerFactory.getLogger(ProjectMember.class);
	...
	/**
	 * Creates an instance of the ProjectMember class. Calls
	 * determineDefaultCompileType.
	 * 
	 * Determines the compileType from the sourceType property of the Member
	 * object.
	 * 
	 * @param project
	 *            Project The project that this Member object is part of.
	 * @param member
	 *            Member The member that is joining the Project.
	 */
	 ...
	 * 
	 * @param project
	 *            Project The project that this member object is a part of.
	 * @param member
	 *            Member The member that is joining the projct.
	 * @param compileType
	 *            String The AS400 command used to compile the source member.
	 * @param destinationLibrary
	 *            String The library to place the compiled program into.
	 */
	...
	/**
	 * move this to projectmember class. the textComponent is optional, if the
	 * source is being displayed in a text component, then include it, so that
	 * compile result line numbers can be matched up to a text component.
	 */
	public void compile(JTextComponent textComponent) {
		...
		cmd = compileType + " " + type + "(" + destinationLibrary + "/" + member.member + ") SRCFILE(" + member.library
				+ "/" + member.file + ") SRCMBR(" + member.member + ") " + option;
		...
		try {
			...
			if (option.length() > 0) {
				...
				buffer = member.as400system.getErrorText(destinationLibrary, "EVFEVENT", member.member,
						member.as400system.name + " " + member.library + " " + member.file + " " + member.member);
				// build the line index if the type is sql
				if (compileType.equals("CRTSQLRPGI")) {
					cmd = "CPYF FROMFILE(QTEMP/QSQLTEMP1) TOFILE(" + destinationLibrary + "/QSQLTEMP1) FROMMBR("
							+ member.member + ") TOMBR(" + member.member + ") MBROPT(*REPLACE) CRTFILE(*YES)";
					member.as400system.call(cmd);
					buffer = member.as400system.indexLineNumbers(destinationLibrary, "QSQLTEMP1", member.member,
							buffer);
				}
